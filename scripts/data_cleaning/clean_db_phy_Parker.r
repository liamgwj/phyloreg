...
setwd("/home/liam/Documents/MSc/analysis/phyloreg/indata/Parker")
...
db <- read.csv("eva265-sup-0001-datas1.csv")

# host phylogeny from Parker
phy <- read.tree("41586_2015_BFnature14372_MOESM285_ESM.txt")

# plant family-genus table according to TPL
tplfg <- read.csv("tpl_fg.csv")
...
write.tree(phy,
           paste0("cleaned_Parker_phylogeny_",
                  Sys.Date(),
                  ".nwk"))

write.csv(db,
          paste0("cleaned_Parker_database_",
                 Sys.Date(),
                 ".csv"),
          row.names = TRUE)
